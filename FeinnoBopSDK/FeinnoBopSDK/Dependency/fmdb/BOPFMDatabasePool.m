...
@interface BOPFMDatabasePool()

- (void)pushDatabaseBackInPool:(BOPFMDatabase*)db;
- (BOPFMDatabase*)db;

@end


@implementation BOPFMDatabasePool
...
+ (instancetype)databasePoolWithPath:(NSString*)aPath flags:(int)openFlags {
    return BOPFMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        ...
        _databaseInPool     = BOPFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = BOPFMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    BOPFMDBRelease(_databaseInPool);
    BOPFMDBRelease(_databaseOutPool);
    
    if (_lockQueue) {
        BOPFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(BOPFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The BOPFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (BOPFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [BOPFMDatabase databaseWithPath:self->_path];
