...
#import "BOPFMDatabasePool.h"
    ...
    #define BOPFMDBAutorelease(__v) ([__v autorelease]);
    #define BOPFMDBReturnAutoreleased BOPFMDBAutorelease

    #define BOPFMDBRetain(__v) ([__v retain]);
    #define BOPFMDBReturnRetained BOPFMDBRetain

    #define BOPFMDBRelease(__v) ([__v release]);

    #define BOPFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define BOPFMDBAutorelease(__v)
    #define BOPFMDBReturnAutoreleased(__v) (__v)

    #define BOPFMDBRetain(__v)
    #define BOPFMDBReturnRetained(__v) (__v)

    #define BOPFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define BOPFMDBDispatchQueueRelease(__v)
    #else
        #define BOPFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^BOPFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in BOPFMDB are:

 - `BOPFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<BOPFMResultSet>` - Represents the results of executing a query on an `BOPFMDatabase`.
 - `<BOPFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<BOPFMDatabasePool>` - A pool of `BOPFMDatabase` objects.
 - `<BOPFMStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [BOPFMDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [BOPFMDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `BOPFMDatabase` object and use it across multiple threads. Instead, use `<BOPFMDatabaseQueue>`.

 */

@interface BOPFMDatabase : NSObject  {
...
/** Create a `BOPFMDatabase` object.
 
 An `BOPFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `BOPFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `BOPFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    BOPFMDatabase *db = [BOPFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    BOPFMDatabase *db     = [BOPFMDatabase databaseWithPath:dbPath];
 ...
 @return `BOPFMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `BOPFMDatabase` object.
...
/** Execute select statement
 ...
 Executing queries returns an `<BOPFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[BOPFMResultSet next]>`) from one record to the other.
 ...
 @return A `<BOPFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see BOPFMResultSet
 @see [`BOPFMResultSet next`](<[BOPFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 */
...
- (BOPFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
/** Test to see if the library is threadsafe
 ...
 @return `NO` if and only if SQLite was compiled with mutexing code omitted due to the SQLITE_THREADSBOPAFE compile-time option being set to 0.
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(BOPFMDatabase *adb) {
        ...
        int rowCount = 0;
        BOPFMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            NSLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        BOPFMDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [BOPFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one BOPFMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using BOPFMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to BOPFMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using BOPFMDB you will not need to interact directly with `BOPFMStatement`, but rather with `<BOPFMDatabase>` and `<BOPFMResultSet>` only.
...
@interface BOPFMStatement : NSObject {
