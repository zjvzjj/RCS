...
{
    uint16_t seqId = (arc4random() % 32767) + 1;
                     ...
                     tag:(long)tag
{
    ...
    if ((bufferData == nil || [bufferData length] == 0) && dataLength < 13)
    {
        // 数据过短，无法解析出包头信息
        combineFlag = YES;
        cmdValue = 0;
    ...
    else
    {
        if (combineFlag)
        {
            [self combineData:data fullLength:packetLength cmd:cmdValue seqId:seqValue];
        }
        else
        {
            uint16_t length = 0;
            ...
            {
                [self finishGetWholeData:data cmd:cmd seqId:seq];
            ...
            else if (length < dataLength)
            {
                ...
                seqValue = 0;
                packetLength = 0;
                NSData *leftData = [data subdataWithRange:NSMakeRange(length, dataLength - length)];
                [self handleBufferData:leftData tag:tag];
            }
            else if (length > dataLength)
            {
                //包被拆分了，需等待下一个包进来合并
...
- (void)combineData:(NSData *)data
         fullLength:(uint16_t)fullLength
                cmd:(uint32_t)cmd
              seqId:(uint16_t)seqId
{
    [bufferData appendData:data];
    if (cmd == 0 || seqId == 0 || fullLength == 0)
    {
        [bufferData getBytes:&fullLength range:NSMakeRange(0, 2)];
        [bufferData getBytes:&cmd range:NSMakeRange(7, 4)];
        [bufferData getBytes:&seqId range:NSMakeRange(11, 2)];
        packetLength = fullLength;
        cmdValue = cmd;
        seqValue = seqId;
    }
...
//    NSLog(@"\nSocket data info:\n dataLength:%lu\n bufferDataLength:%lu\n fullLength:%d\n seq:%d\n\n", (unsigned long)data.length, (unsigned long)bufferData.length ,fullLength, seqId);
    ...
    uint16_t bufferDataLength = (uint16_t)bufferData.length;
    ...
    else if (bufferDataLength > fullLength)
    {
        ...
        uint16_t leftLength = bufferDataLength - fullLength;
