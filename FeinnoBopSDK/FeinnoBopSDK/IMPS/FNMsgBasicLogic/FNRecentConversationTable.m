...
+ (NSMutableArray *)get
{
    ...
    [queue inDatabase:^(BOPFMDatabase *db) {
        ...
        while ([rs next])
        {
            ...
            recentConversation.targetId = [rs stringForColumnIndex:0];
...
+ (NSMutableArray *)getByTid:(NSString *)tid
{
    ...
    [queue inDatabase:^(BOPFMDatabase *db) {
        ...
        BOPFMResultSet *rs = [db executeQuery:sql, [Utility userIdWithoutAppKey:tid]];
        while ([rs next])
        {
            FNRecentConversationTable *recentConversation = [[FNRecentConversationTable alloc] init];
            recentConversation.targetId = [rs stringForColumnIndex:0];
...
+ (BOOL)updateContent:(NSString *)msgContent tid:(NSString *)tid
{
    ...
    [queue inDatabase:^(BOPFMDatabase *db) {
        ...
        result = [db executeUpdate:sql, msgContent, [Utility userIdWithoutAppKey:tid]];
    }];
    return result;
}
...
+ (BOOL)insert:(FNRecentConversationTable *)info
{
    ...
    [queue inDatabase:^(BOPFMDatabase *db) {
        ...
        result = [db executeUpdate:sql,
                  [Utility userIdWithoutAppKey:info.targetId],
                  info.targetName,
                  info.senderNickname,
...
+ (BOOL)delete:(NSString *)targetId
{
    ...
    [queue inDatabase:^(BOPFMDatabase *db) {
        ...
        result = [db executeUpdate:sql, [Utility userIdWithoutAppKey:targetId]];
    }];
    return result;
}   
+ (BOOL)clearAll
...
+ (BOOL)updateUnReadCount:(NSString *)targetId
{
    ...
    [queue inDatabase:^(BOPFMDatabase *db) {
        ...
        result = [db executeUpdate:sql, @0L, [Utility userIdWithoutAppKey:targetId]];
    }];
    return result;
}
...
- (int64_t)setUnreadMsgCount
{
    ...
    [queue inDatabase:^(BOPFMDatabase *db) {
        ...
        _unreadMsgCount = [db intForQuery:sql, [Utility userIdWithoutAppKey:_targetId], [NSNumber numberWithInt:MsgUnread]];
    }];
