...
@class BOPGDataXMLElement, BOPGDataXMLDocument;


static const int kBOPGDataXMLParseOptions = (XML_PARSE_NOCDATA | XML_PARSE_NOBLANKS);
...
// BOPGDataXMLNode does not require pulling in all of GData.
...
static NSString *GDataFakeQNameForURIAndName(NSString *theURI, NSString *name) {
  ...
  NSString *localName = [BOPGDataXMLNode localNameForName:name];
...
@interface BOPGDataXMLNode (PrivateMethods)
...
- (BOPGDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode;
...
+ (BOPGDataXMLElement *)elementWithName:(NSString *)name {
...
+ (BOPGDataXMLElement *)elementWithName:(NSString *)name stringValue:(NSString *)value {
...
+ (BOPGDataXMLElement *)elementWithName:(NSString *)name URI:(NSString *)theURI {
...
+ (id)nodeConsumingXMLNode:(xmlNodePtr)theXMLNode {
  ...
  if (theXMLNode->type == XML_ELEMENT_NODE) {
    theClass = [BOPGDataXMLElement class];
  } else {
    theClass = [BOPGDataXMLNode class];
  }
...
#if DEBUG
  NSCAssert(chars != NULL, @"BOPGDataXMLNode sees an unexpected empty string");
#endif
...
- (NSArray *)children {
  ...
  if (xmlNode_ != NULL) {
    ...
    while (currChild != NULL) {
      BOPGDataXMLNode *node = [BOPGDataXMLNode nodeBorrowingXMLNode:currChild];
...
- (BOPGDataXMLNode *)childAtIndex:(unsigned)index {
...
- (BOPGDataXMLNodeKind)kind {
  if (xmlNode_ != NULL) {
    xmlElementType nodeType = xmlNode_->type;
    switch (nodeType) {
      case XML_ELEMENT_NODE:         return BOPGDataXMLElementKind;
      case XML_ATTRIBUTE_NODE:       return BOPGDataXMLAttributeKind;
      case XML_TEXT_NODE:            return BOPGDataXMLTextKind;
      case XML_CDATA_SECTION_NODE:   return BOPGDataXMLTextKind;
      case XML_ENTITY_REF_NODE:      return BOPGDataXMLEntityDeclarationKind;
      case XML_ENTITY_NODE:          return BOPGDataXMLEntityDeclarationKind;
      case XML_PI_NODE:              return BOPGDataXMLProcessingInstructionKind;
      case XML_COMMENT_NODE:         return BOPGDataXMLCommentKind;
      case XML_DOCUMENT_NODE:        return BOPGDataXMLDocumentKind;
      case XML_DOCUMENT_TYPE_NODE:   return BOPGDataXMLDocumentKind;
      case XML_DOCUMENT_FRAG_NODE:   return BOPGDataXMLDocumentKind;
      case XML_NOTATION_NODE:        return BOPGDataXMLNotationDeclarationKind;
      case XML_HTML_DOCUMENT_NODE:   return BOPGDataXMLDocumentKind;
      case XML_DTD_NODE:             return BOPGDataXMLDTDKind;
      case XML_ELEMENT_DECL:         return BOPGDataXMLElementDeclarationKind;
      case XML_ATTRIBUTE_DECL:       return BOPGDataXMLAttributeDeclarationKind;
      case XML_ENTITY_DECL:          return BOPGDataXMLEntityDeclarationKind;
      case XML_NAMESPACE_DECL:       return BOPGDataXMLNamespaceKind;
      case XML_XINCLUDE_START:       return BOPGDataXMLProcessingInstructionKind;
      case XML_XINCLUDE_END:         return BOPGDataXMLProcessingInstructionKind;
      case XML_DOCB_DOCUMENT_NODE:   return BOPGDataXMLDocumentKind;
    }
  }
  return BOPGDataXMLInvalidKind;
}
...
- (NSArray *)nodesForXPath:(NSString *)xpath
  ...
  // are created from BOPGDataXMLElement's initWithXMLString there may not be
  // a document. (We may later decide that we want to stuff the doc used
  // there into a BOPGDataXMLDocument and retain it, but we don't do that now.)
  //
    ...
    if (xpathCtx) {
      ...
      if (namespaces) {
        ...
        for (NSString *prefix in namespaces) {
          ...
          if (result != 0) {
#if DEBUG
            NSCAssert1(result == 0, @"BOPGDataXMLNode XPath namespace %@ issue",
                      prefix);
#endif
          }
        }
        ...
        if (nsNodePtr != NULL) {
          for (xmlNsPtr nsPtr = nsNodePtr->ns; nsPtr != NULL; nsPtr = nsPtr->next) {
            ...
            if (prefix == NULL) {
              prefix = (xmlChar*) kBOPGDataXMLXPathDefaultNamespacePrefix;
            }

            int result = xmlXPathRegisterNs(xpathCtx, prefix, nsPtr->href);
            if (result != 0) {
#if DEBUG
              NSCAssert1(result == 0, @"BOPGDataXMLNode XPath namespace %s issue",
                        prefix);
#endif
            }
          }
      ...
      if (xpathObj) {
        ...
        if (nodeSet) {
          ...
          for (int index = 0; index < nodeSet->nodeNr; index++) {
            ...
            BOPGDataXMLNode *node = [BOPGDataXMLNode nodeBorrowingXMLNode:currNode];
            if (node) {
              [array addObject:node];
            }
          }
  ...
  if (array == nil && error != nil) {
    *error = [NSError errorWithDomain:@"com.google.BOPGDataXML"
                                 code:errorCode
                             userInfo:errorInfo];
  }
...
- (BOOL)isEqual:(BOPGDataXMLNode *)other {
  if (self == other) return YES;
  if (![other isKindOfClass:[BOPGDataXMLNode class]]) return NO;
...
- (NSUInteger)hash {
  return (NSUInteger) (void *) [BOPGDataXMLNode class];
}
...
@implementation BOPGDataXMLElement
  ...
  if (xmlNode_ != NULL && xmlNode_->nsDef != NULL) {
    ...
    while (currNS != NULL) {
      ...
      if (!xmlStrEqual(currNS->prefix, (const xmlChar *) "xml")) {
        BOPGDataXMLNode *node = [BOPGDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) currNS];
...
- (void)addNamespace:(BOPGDataXMLNode *)aNamespace {
...
- (void)addChild:(BOPGDataXMLNode *)child {
  if ([child kind] == BOPGDataXMLAttributeKind) {
...
- (void)removeChild:(BOPGDataXMLNode *)child {
...
- (NSArray *)elementsForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    for (BOPGDataXMLNode *child in children) {
...
- (NSArray *)attributes {
  ...
  if (xmlNode_ != NULL && xmlNode_->properties != NULL) {
    ...
    while (prop != NULL) {
      ...
      BOPGDataXMLNode *node = [BOPGDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) prop];
      if (array == nil) {
        array = [NSMutableArray arrayWithObject:node];
      } else {
        [array addObject:node];
...
- (void)addAttribute:(BOPGDataXMLNode *)attribute {
...
- (BOPGDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode {
  // search the cached attributes list for the BOPGDataXMLNode with
  // the underlying xmlAttrPtr
  NSArray *attributes = [self attributes];

  for (BOPGDataXMLNode *attr in attributes) {
...
- (BOPGDataXMLNode *)attributeForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (attrPtr) {
      BOPGDataXMLNode *attr = [self attributeForXMLNode:attrPtr];
      return attr;
    }
  }
  return nil;
...
- (BOPGDataXMLNode *)attributeForLocalName:(NSString *)localName
...
@interface BOPGDataXMLDocument (PrivateMethods)
- (void)addStringsCacheToDoc;
@end

@implementation BOPGDataXMLDocument

- (id)initWithXMLString:(NSString *)str options:(unsigned int)mask error:(NSError **)error {

  NSData *data = [str dataUsingEncoding:NSUTF8StringEncoding];
  BOPGDataXMLDocument *doc = [self initWithData:data options:mask error:error];
  return doc;
}
...
- (id)initWithData:(NSData *)data options:(unsigned int)mask error:(NSError **)error {
  ...
  if (self) {
    ...
    xmlDoc_ = xmlReadMemory((const char*)[data bytes], (int)[data length], baseURL, encoding,
                            kBOPGDataXMLParseOptions); // TODO(grobbins) map option values
    if (xmlDoc_ == NULL) {
      if (error) {
       *error = [NSError errorWithDomain:@"com.google.BOPGDataXML"
                                    code:-1
...
- (id)initWithRootElement:(BOPGDataXMLElement *)element {
...
- (void)addStringsCacheToDoc {
  ...
  NSCAssert(xmlDoc_ != NULL && xmlDoc_->_private == NULL,
            @"BOPGDataXMLDocument cache creation problem");
#endif
...
- (BOPGDataXMLElement *)rootElement {
  BOPGDataXMLElement *element = nil;

  if (xmlDoc_ != NULL) {
    xmlNodePtr rootNode = xmlDocGetRootElement(xmlDoc_);
    if (rootNode) {
      element = [BOPGDataXMLElement nodeBorrowingXMLNode:rootNode];
    }
  }
  return element;
}
...
- (NSArray *)nodesForXPath:(NSString *)xpath
                namespaces:(NSDictionary *)namespaces
                     error:(NSError **)error {
  if (xmlDoc_ != NULL) {
    BOPGDataXMLNode *docNode = [BOPGDataXMLElement nodeBorrowingXMLNode:(xmlNodePtr)xmlDoc_];
