...
const int32_t PROTO_BUFFER_SIZE = 4096;
...
- (id) initWithInputStream:(NSInputStream*) input_ {
  if ((self = [super init])) {
    self.buffer = [NSMutableData dataWithLength:PROTO_BUFFER_SIZE];
    bufferSize = 0;
    self.input = input_;
    [input open];
    [self commonInit];
...
- (NSData*) readRawData:(int32_t) size {
  ...
  if (size <= bufferSize - bufferPos) {
    ...
    bufferPos += size;
    return data;
  } else if (size < PROTO_BUFFER_SIZE) {
    ...
    while (sizeLeft > 0) {
      NSMutableData* chunk = [NSMutableData dataWithLength:MIN(sizeLeft, PROTO_BUFFER_SIZE)];
