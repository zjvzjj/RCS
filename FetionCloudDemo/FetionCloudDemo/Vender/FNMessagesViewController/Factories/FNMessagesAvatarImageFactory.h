...
/**
 *  `FNMessagesAvatarImageFactory` is a factory that provides a means for creating and styling
 *  `FNMessagesAvatarImage` objects to be displayed in a `FNMessagesCollectionViewCell` of a `FNMessagesCollectionView`.
 */
@interface FNMessagesAvatarImageFactory : NSObject

/**
*  Creates and returns a `FNMessagesAvatarImage` object with the specified placeholderImage that is
...
*  @param diameter 直径        An integer value specifying the diameter size of the avatar in points. This value must be greater than `0`.
*
*  @return An initialized `FNMessagesAvatarImage` object if created successfully, `nil` otherwise.
*/
+ (FNMessagesAvatarImage *)avatarImageWithPlaceholder:(UIImage *)placeholderImage diameter:(NSUInteger)diameter;

/**
 *  Creates and returns a `FNMessagesAvatarImage` object with the specified image that is
 *  cropped to a circle of the given diameter and used for the `avatarImage` and `avatarPlaceholderImage` properties
 *  of the returned `FNMessagesAvatarImage` object. This image is then copied and has a transparent black mask applied to it, 
 *  which is used for the `avatarHighlightedImage` property of the returned `FNMessagesAvatarImage` object.
 *
...
/**
 *  Creates and returns a `FNMessagesAvatarImage` object with a circular shape that displays the specified userInitials 首字母
